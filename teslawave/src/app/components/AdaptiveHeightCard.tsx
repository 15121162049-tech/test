...
export default function AdaptiveHeightCard() {
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      const cardWidth = 480; // 原始宽度，transform不影响布局
      const baseHeight = collapsedHeightRef.current; // 原始高度

      // 考虑 scale(0.8) 的影响
      const effectiveTop = STATUS_BAR_HEIGHT + 10; // 视觉顶部距离状态栏10px
      const effectiveBottom = containerHeight - DOCK_HEIGHT - 10; // 视觉底部距离Dock栏10px
      ...
      if (snapPosition === 'top-left') {
        newLeftX = PADDING;
        newCenterY = effectiveTop + baseHeight * 0.4; // centerY = 视觉顶部 + 原始高度 * 0.4
        ...
        newLeftX = PADDING;
        newCenterY = effectiveBottom - baseHeight * 0.4; // centerY = 视觉底部 - 原始高度 * 0.4
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 bg-white/10 backdrop-blur-xl">
        <div className="absolute top-1/3 left-1/3 w-96 h-96 bg-blue-500/10 rounded-full blur-3xl opacity-80"></div>
        <div className="absolute bottom-1/3 right-1/3 w-80 h-80 bg-cyan-500/8 rounded-full blur-3xl opacity-80"></div>
      ...
      <div className="relative z-10">
        ...
        <div className="absolute top-2.5 right-2.5 z-50">
          <button
            ...
            className="w-[24px] h-[24px] bg-white/15 hover:bg-white/25 rounded-full backdrop-blur-md transition-colors duration-200 flex items-center justify-center"
            aria-label="关闭"
          >
        ...
        {/* 内容区 */}
        <div className="relative p-4 pt-12 pb-16 max-h-[calc(100vh-2rem)] overflow-y-auto">
